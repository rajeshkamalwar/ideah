...
class BuyPlanController extends Controller
{
    public function index(Request $request)
    {
        if (!VendorPermissionHelper::subscriptionPlansEnabled()) {
            Session::flash('warning', __('Subscription plans are disabled by the administrator.'));
        ...
        $data['claim'] = $request->query('claim');
        $data['t']     = $request->query('t');

        return view('vendors.buy_plan.index', $data);
    }

    public function checkout($package_id, Request $request)
    {
        if (!VendorPermissionHelper::subscriptionPlansEnabled()) {
            Session::flash('warning', __('Subscription plans are disabled by the administrator.'));
