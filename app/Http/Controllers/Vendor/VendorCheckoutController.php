...
class VendorCheckoutController extends Controller
{
    public function checkout(ExtendRequest $request)
    {
        try {
            ...
            if ($request->price == 0) {
                ...
                // changing the currency before redirect to Paytabs
                ...
                // changing the currency before redirect to PhonePe
                if ($bs->base_currency_text != 'INR') {
                ...
                // changing the currency before redirect to Toyyibpay
                ...
                // changing the currency before redirect to Yoco
                ...
                // if ($request->filled(['claim', 't'])) {
                //     $request['claim'] = (int)$request->input('claim');
    ...
    public function store($request, $transaction_id, $transaction_details, $amount, $be, $password)
    {
        ...
        if ($vendor) {
            $membership = Membership::create([
                ...
                'claim_id' => array_key_exists('claim', $request) ? (int)$request['claim'] : null,
    ...
    /**
     * Standalone success URLs for membership flows (e.g. redirects/bookmarks).
     */
