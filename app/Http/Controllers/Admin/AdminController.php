...
class AdminController extends Controller
{
    ...
    {
        $adminLoginPrefill = $this->adminLoginPrefillData($request);

        return view('admin.login', compact('adminLoginPrefill'));
    }

    /**
     * Optional username/password prefill for /admin when ADMIN_LOGIN_PREFILL is enabled
     * and the request host is in ADMIN_LOGIN_PREFILL_HOSTS.
     */
    protected function adminLoginPrefillData(Request $request): array
    {
        $empty = ['username' => '', 'password' => ''];
        $cfg = config('app.admin_login_prefill', []);
        ...
        if ($hosts === [] || !in_array($request->getHost(), $hosts, true)) {
            return $empty;
        }
