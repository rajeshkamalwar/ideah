...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Map items to include product and vendor information
        $items->map(function ($item) {
            ...
            $item['vendor_name'] = $product->vendor_id ? Vendor::where('id', $product->vendor_id)->value('username') : 'Admin';
            return $item;
        });

        $information['items'] = $items;

        // Decode vendor details from per_vendor_discount_and_commission (if JSON)
        $information['vendor_details'] = $order->per_vendor_discount_and_commission
            ? json_decode($order->per_vendor_discount_and_commission, true)
            : [];

        $information['total_commission'] = number_format($order->total_commission, 2, '.', '');
        $information['admin_amount_with_commission'] = number_format($order->admin_amount_with_commission, 2, '.', '');
    ...
    {
        $information['onlineGateways'] = OnlineGateway::query()->where('status', '=', 1)->get();
        $information['offlineGateways'] = OfflineGateway::query()->where('status', '=', 1)->orderBy('serial_number', 'asc')->get();
        ...
        if ($request->filled('from') && $request->filled('to')) {
            ...
            $information['orders'] = $collection_2;
        } else {
            Session::put('product_orders', null);
            $information['orders'] = [];
        }

        return view('admin.shop.order.report', $information);
    }
