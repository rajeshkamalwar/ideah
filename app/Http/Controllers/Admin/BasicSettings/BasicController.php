...
class BasicController extends Controller
{
    public function setLocaleAdmin(Request $request)
    {
        ...
        $admin->lang_code = 'admin_' . $request->code;
    ...
    public function updateFavicon(Request $request)
    {
        ...
        if ($request->hasFile('favicon')) {
            ...
            Session::flash('success', __('Favicon updated successfully') . '!');
        }
    ...
    public function updateThemeAndHome(Request $request)
    {
        ...
        Session::flash('success', __('Theme & home version updated successfully') . '!');
    ...
    public function updateAppearance(Request $request)
    {
        ...
        Session::flash('success', __('Appearance updated successfully') . '!');
    ...
    public function updateBreadcrumb(Request $request)
    {
        ...
        Session::flash('success', __('Breadcrumb updated successfully') . '!');
    ...
    public function updateGoogleMapApi(Request $request)
    {
        $rules = [
        ...
        $current = DB::table('basic_settings')->where('uniqid', 12345)->first();
        $submitted = trim((string) $request->input('google_map_api_key', ''));
        // Form used to show a fake value "xxxxxx"; saving would overwrite the real key.
        $placeholderSent = ($submitted === '' || $submitted === 'xxxxxx');
        $existingKey = $current && !empty($current->google_map_api_key) ? (string) $current->google_map_api_key : '';

        if ($placeholderSent) {
            if ($existingKey === '') {
                return redirect()->back()->withErrors([
                    'google_map_api_key' => __('Please enter your Google Maps API key.'),
                ]);
            }
            $apiKey = $existingKey;
        } else {
        ...
        Session::flash('success', __('Google map api updated successfully') . '!');
        return redirect()->back();
    }
    ...
    {
        $info = DB::table('basic_settings')->select('admin_approve_status')->first();
        return view('admin.basic-settings.admin-approve', ['info' => $info]);
    }

    public function updateAdminApprove(Request $request)
    {
        $rules = [
    ...
    {
        $data = DB::table('basic_settings')->select('product_tax_amount', 'commission_amount')->first();
    ...
    {
        $rules = [
            'product_tax_amount' => 'required|numeric|min:0',
        ...
        // Store the tax and commission amount info into db
        DB::table('basic_settings')->updateOrInsert(
            ['uniqid' => 12345],
            [
        ...
        Session::flash('success', __('Tax and commission amounts updated successfully') . '!');
    ...
    public function pwa(Request $request)
    {
        ...
        return view('admin.basic-settings.pwa', $data);
    }
    ...
    public function update_general_setting(Request $request)
    {
        ...
        Session::flash('success', __('General settings update Successfully') . '!');
