          ...
          <div class="row">
            ...
            <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
              <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                data-href="{{ route('vendor.product.message.bulk_delete.message') }}">
                        ...
                        <tr>
                          ...
                          @php
                            ...
                            if ($product) {
                                ...
                                $product_content = $product->content->where('language_id', $language->id)->first();

                                if ($product->listing_id) {
                                    $listing = App\Models\Listing\Listing::with(['listing_content' => function($query) use ($language) {
                          ...
                          <td class="title">
                            ...
                            <a href="{{ route('shop.product_details', ['slug' => $product_content->slug]) }}" target="_blank">
                              {{ strlen($product_content->title) > 40 ? mb_substr($product_content->title, 0, 40, 'utf-8') . '...' : $product_content->title }}
                            </a>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm mt-1 mr-1" href="{{ route('vendor.product.message.details', $message->id) }}" target="_blank">
                            ...
                            <form class="deleteForm d-inline-block"
                              action="{{ route('vendor.product.message.delete_message') }}" method="post">
                              @csrf
