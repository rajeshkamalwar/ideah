            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title d-inline-block">
                        {{ __('Add Product') . ' (' . __('Type') . ' - ' . __($productType) . ')' }}
                    </div>
                    <a class="btn btn-info btn-sm float-right d-inline-block"
                        href="{{ route('vendor.shop_management.select_product_type') }}">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-8 offset-lg-2">
                            ...
                            <form id="productForm" action="{{ route('vendor.shop_management.store_product') }}"
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <select name="placement_type" class="form-control">
                                                ...
                                                <option value="1">{{ __('Marketplace') }}</option>
                                                <option value="2">{{ __('Listing') }}</option>
                                                <option value="3">{{ __('Both (Marketplace & Listing)') }}</option>
                                            </select>
                                            ...
                                            <select name="listing_id" class="form-control">
                                                ...
                                                @foreach ($listings as $listing)
                                                    @if ($listing->listing_content->isNotEmpty())
                                                        <option value="{{ $listing->id }}">{{ $listing->listing_content->first()->title }}</option> 
                                                    @endif                                                   
                                                @endforeach
                                            </select>
                                        </div>
...
@section('script')
    <script>
        'use strict';
        const imgUpUrl = "{{ route('vendor.shop_management.upload_slider_image') }}";
        const imgRmvUrl = "{{ route('vendor.shop_management.remove_slider_image') }}";
        const maxSliderImage = {{ $remaingSliderImage }};
    </script>
