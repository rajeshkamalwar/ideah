...
@section('content')
                        ...
                        <tr>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm mr-1  mt-1 btn-sm editBtn" href="#"
                              data-toggle="modal" data-target="#editModal" data-id="{{ $charge->id }}"
                              data-title="{{ $charge->title }}" data-short_text="{{ $charge->short_text }}"
                              data-shipping_charge="{{ $charge->shipping_charge }}"
                              data-serial_number="{{ $charge->serial_number }}">
                            ...
                            <form class="deleteForm d-inline-block"
                              action="{{ route('vendor.shop_management.delete_charge', ['id' => $charge->id]) }}"
                              method="post">
                              @csrf
  ...
  {{-- create modal --}}
  @include('vendors.shop.shipping-charge.create')

  {{-- edit modal --}}
  @include('vendors.shop.shipping-charge.edit')
@endsection
