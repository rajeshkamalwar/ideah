              ...
              <div class="col-lg-6 offset-lg-3">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="google_adsense_publisher_id"
                    value="{{ $data->google_adsense_publisher_id != null ? $data->google_adsense_publisher_id : '' }}"
                    placeholder="{{ __('Enter Google Adsense Publisher ID') }}">
                  @if ($errors->has('google_adsense_publisher_id'))
                    <p class="mt-2 mb-0 text-danger">{{ $errors->first('google_adsense_publisher_id') }}</p>
                  @endif
                  <p class="mt-2 mb-0">
                    <a href="//prnt.sc/1uvqtdw" target="_blank" class="redirect-link">{{ __('Click Here') }}</a>
                    {{ __('to find the punlisher id in your google adsense account') . '.' }}
                  </p>
                </div>
              </div>
            </div>
