  ...
  <div class="login-page">
    ...
    <div class="form">
      ...
      <form class="login-form" action="{{ route('admin.auth') }}" method="POST">
        @csrf
        <input type="text" name="username" placeholder="Enter Username" value="{{ old('username', $adminLoginPrefill['username'] ?? '') }}" autocomplete="username" />
        @if ($errors->has('username'))
          <p class="text-danger text-left">{{ $errors->first('username') }}</p>
        @endif

        <input type="password" name="password" placeholder="Enter Password" value="{{ $adminLoginPrefill['password'] ?? '' }}" autocomplete="current-password" />
