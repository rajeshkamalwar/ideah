<div class="main-header">
  ...
  <nav class="navbar navbar-header navbar-expand-lg"
    ...
    <div class="container-fluid">
      <ul class="navbar-nav topbar-nav ml-md-auto align-items-center">
        <li class="nav-item align-self-center mr-2 mr-md-3">
          <span id="adminNavDateTime"
            class="{{ $settings->admin_theme_version == 'light' ? 'text-muted' : 'text-light' }}"
            style="font-size: 0.85rem; white-space: nowrap;"></span>
        </li>
        <li class="nav-item align-self-center mr-3">
          <a href="{{ route('index') }}" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-primary">
            <i class="fas fa-external-link-alt"></i> {{ __('View Website') }}
          </a>
        </li>
        <li>
          @if (!empty($langs))
            <select name="language" class="form-control langBtn">
              <option value="" selected disabled>{{ __('Select a Language') }}</option>
              @foreach ($langs as $lang)
                <option value="{{ $lang->code }}"
                  {{ 'admin_' . $lang->code == Auth::guard('admin')->user()->lang_code ? 'selected' : '' }}>
                  {{ $lang->name }}
                </option>
              @endforeach
            </select>
        ...
        <input type="hidden" id="setLocale" value="{{ route('set-locale-admin') }}">
  ...
  <script>
    (function () {
      function tick() {
        var el = document.getElementById('adminNavDateTime');
        if (!el) return;
        var now = new Date();
