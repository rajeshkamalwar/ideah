            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="google_map_api_key_status" value="1" class="selectgroup-input"
                        {{ $data->google_map_api_key_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Active') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="google_map_api_key_status" value="0" class="selectgroup-input"
                        {{ $data->google_map_api_key_status == 0 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Deactive') }}</span>
                    </label>

                  </div>
                  <h6 class="mt-2 mb-1 text-warning">{{ __('If the Google Maps API is disabled:') }}</h6>
                  <ul class="pl-20 mb-0">
                    <li>
                    ...
                    <li>
                      <p class="mb-0 text-warning">{{ __('Radius-base searchings will be disabled.') }}
                      </p>
                    </li>
                  </ul>

                  @if ($errors->has('google_map_api_key_status'))
                    <p class="mt-1 mb-0 text-danger">{{ $errors->first('google_map_api_key_status') }}</p>
                  @endif
                </div>
                <div class="form-group">
                  <label>{{ __('Api Key') }}</label>
                  <input type="password" class="form-control" name="google_map_api_key" value=""
                    autocomplete="off"
                    placeholder="{{ !empty($data) && !empty($data->google_map_api_key) ? __('Paste a new key to replace the current one') : __('Paste your Google Maps API key') }}">
                  <p class="mb-0 mt-1 text-muted small">
                    @if (!empty($data) && !empty($data->google_map_api_key))
                      {{ __('A key is already saved. Leave the field empty to keep it, or paste a new key.') }}
                    @else
                      {{ __('Required on first save. Get a key from Google Cloud Console (Maps JavaScript API, Places API, Geocoding API).') }}
                    @endif
                  </p>
