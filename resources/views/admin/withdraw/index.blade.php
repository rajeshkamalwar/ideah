                                    ...
                                    <table class="table table-striped mt-3" id="basic-datatables">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">{{ __('Min Limit') . ' ' }}({{ __('in') }}
                                                    {{ $websiteInfo->base_currency_text }})</th>
                                                <th scope="col">{{ __('Max Limit') . ' ' }}({{ __('in') }}
                                                    {{ $websiteInfo->base_currency_text }})</th>
                                            ...
                                            @foreach ($collection as $item)
                                                <tr>
                                                    ...
                                                    <td><a class="btn btn-info btn-sm"
                                                            href="{{ route('admin.withdraw_payment_method.mange_input', ['id' => $item->id, 'language' => $defaultLang->code]) }}">{{ __('Manage Form') }}</a>
                                                    </td>
                                                    <td>
                                                        @if ($item->status == 1)
                                                    ...
                                                    <td>
                                                        <a class="btn btn-secondary btn-sm mr-1 mb-1 editBtn" href="#"
                                                            data-toggle="modal" data-target="#editModal"
                                                            data-id="{{ $item->id }}" data-name="{{ $item->name }}"
                                                            data-min_limit="{{ $item->min_limit }}"
                                                        ...
                                                        <form class="deleteForm d-inline-block"
                                                            ...
                                                            @csrf
                                                            <button type="submit"
                                                                class="btn btn-danger mb-1 btn-sm deleteBtn">
                                                                <span class="btn-label">
