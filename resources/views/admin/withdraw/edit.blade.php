    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Edit Payment Method') }}</h5>
            ...
            <div class="modal-body">
                ...
                <form id="ajaxEditForm" class="modal-form create"
                    action="{{ route('admin.withdraw_payment_method.update') }}" method="post"
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Minimum Limit') . ' ' }}({{ __('in') }}
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Maximum Limit') . ' ' }}({{ __('in') }}
                            {{ $websiteInfo->base_currency_text }})*</label>
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Fixed Charge') . ' '}} ({{ __('in') }}
                            {{ $websiteInfo->base_currency_text }})</label>
                        <input type="number" class="form-control" value="0.00" id="in_fixed_charge"
                            name="fixed_charge" placeholder="{{ __('Enter Fixed Charge') }}">
                        <p id="editErr_fixed_charge" class="mt-1 mb-0 text-danger em"></p>
                    </div>

                    <div class="form-group">
                        <label for="">{{ __('Percentage Charge')  . ' '}}({{ __('in') }}
                            {{ $websiteInfo->base_currency_text }})</label>
                        <input type="number" class="form-control"value="0.00" id="in_percentage_charge"
                            name="percentage_charge" placeholder="{{ __('Enter Percentage Charge') }}">
