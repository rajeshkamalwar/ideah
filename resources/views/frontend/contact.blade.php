...
@section('content')
  ...
  <div class="contact-area ptb-100">
    <div class="container">
      @if (!empty($info->contact_title) || !empty($info->contact_subtile) || !empty($info->contact_details))
        <div class="row justify-content-center mb-50">
          <div class="col-lg-10 col-xl-8 text-center" data-aos="fade-up">
            @if (!empty($info->contact_title))
              <h2 class="title mb-15">{{ $info->contact_title }}</h2>
            @endif
            @if (!empty($info->contact_subtile))
              <p class="text color-medium mb-20">{{ $info->contact_subtile }}</p>
            @endif
            @if (!empty($info->contact_details))
              <p class="text mb-0 lh-base">{!! nl2br(e($info->contact_details)) !!}</p>
            @endif
          </div>
        </div>
      @endif
      ...
      <div class="row justify-content-center">
        <div class="col-lg-4 col-md-6">
          <div class="card mb-30 color-1" data-aos="fade-up" data-aos-delay="100">
            ...
            <div class="card-text">
              <p class="font-sm color-medium mb-1">{{ __('Phone') }}</p>
              @if (!empty($info->contact_number))
                <p class="mb-0"><a href="tel:{{ preg_replace('/\s+/', '', $info->contact_number) }}">{{ $info->contact_number }}</a></p>
              @endif
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-6">
          <div class="card mb-30 color-2" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="card-text">
              <p class="font-sm color-medium mb-1">{{ __('Email') }}</p>
              @if (!empty($info->email_address))
                <p class="mb-0"><a href="mailto:{{ $info->email_address }}">{{ $info->email_address }}</a></p>
              @endif
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-6">
          <div class="card mb-30 color-3" data-aos="fade-up" data-aos-delay="300">
            ...
            <div class="card-text">
              <p class="font-sm color-medium mb-1">{{ __('Address') }}</p>
      ...
      @if (!empty($info->kvk_number))
        <div class="row justify-content-center mt-20 mb-10">
          <div class="col-lg-8 text-center" data-aos="fade-up">
            <p class="text mb-0 font-sm color-medium">
              <i class="fal fa-file-certificate color-primary me-1"></i>
              {{ __('KVK') }} {{ __('Number') }}: <strong>{{ $info->kvk_number }}</strong>
            </p>
          </div>
        </div>
      @endif
